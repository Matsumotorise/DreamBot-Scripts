...
public class minerGui extends JFrame {
	...
	private final JCheckBox chckbxDontMove = new JCheckBox("Don't Move");
	...
	public minerGui(final ScriptVars var, final MethodContext ctx) {
		...
		setIconImage(Toolkit.getDefaultToolkit()
				.getImage(minerGui.class.getResource("/javax/swing/plaf/metal/icons/ocean/computer.gif")));
		setAlwaysOnTop(true);
		setResizable(false);
		...
		comboBox.setModel(new DefaultComboBoxModel<>(BankLocation.values()));
		...
		if (ctx.getClient().isLoggedIn()) {
			myTile = ctx.getLocalPlayer().getTile();
		}
		if (myTile != null) {
		...
		btnSave.addActionListener(arg0 -> saveFile(var, txtFilename.getText()));
		btnSave.setBounds(106, 248, 57, 23);
		contentPane.add(btnSave);

		JButton btnLoad = new JButton("Load");
		btnLoad.addActionListener(arg0 -> loadFile(var, ctx, txtFilename.getText()));
	...
	private void loadFile(final ScriptVars var, final MethodContext ctx, String fileName) {
		var.tasks.clear();
		model1.clear();
		...
		for (int i = 0; i < content.length; i += 7) {
			...
			Tile startTile = new Tile(Integer.parseInt(tileStringSplit[0]), Integer.parseInt(tileStringSplit[1]),
					Integer.parseInt(tileStringSplit[2]));
			//get goal
